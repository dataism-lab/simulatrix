...
def reward_function(state: Dict) -> float:
    ...
    # Input parameters
    on_track = state['all_wheels_on_track']
    distance_from_center = state['distance_from_center']
    keep_left = state['is_left_of_center']
    track_width = state['track_width']
    steering = abs(
        state['steering_angle'])  # Only need the absolute steering angle for calculations
    speed = state['speed']
    waypoints = state['waypoints']
    closest_waypoints = state['closest_waypoints']
